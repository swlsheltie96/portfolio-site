<script lang="ts">
  ...
  const pageTitle = $derived(
    `${project.title} — ${data.siteSettings?.title ?? "SWL Studio"}`,
  );
  const pageDescription = $derived(
    project.excerpt ??
      `${project.title}, ${project.year}. ${project.categories?.join(", ") ?? ""}`,
  );
  ...
  {#if project.gallery?.length}
    <div class="gallery" class:stacked={project.stackGallery}>
      {#each project.gallery as item (item._key)}
        {@const desktopCol =
          item.width === "quarter" ? 4 : item.width === "half" ? 8 : 16}
        {@const tabletCol =
          item.width === "quarter" ? 3 : item.width === "half" ? 5 : 10}
        {@const colClass = `col-${desktopCol} col-tablet-${tabletCol} col-mobile-5`}
        {@const imageWidth =
          item.width === "quarter" ? 600 : item.width === "half" ? 1200 : 1600}
        {@const sizes =
          item.width === "quarter"
      ...
      {/each}
      <div
        class="all-projects related component col-16 col-tablet-10 col-mobile-5"
      >
        <ProjectsListSmall
