...
<div class="projects-list-wrapper">
	...
	<div class="header-bar text-body">
		...
		{#if showPreview}
			<span class="col-2 col-tablet-3 col-mobile-2">Preview</span>
		{/if}
	</div>
				...
				{#if project.comingSoon}
					...
					>
						...
						<span class="project-title">
							...
							<span class="title-hover">{project.comingSoonText || 'Coming Soon'}</span>
						</span>
						<span class="col-2 col-tablet-2 col-mobile-1">{project.year}</span>
						<span class="col-3 col-tablet-3 col-mobile-2 type-col">{project.categories?.join(', ')}</span>
						{#if showPreview}
							<span class="preview-thumb col-2 col-tablet-3 col-mobile-2">
								{#if project.coverVideo}
									<video src={project.coverVideo} autoplay muted loop playsinline></video>
								{:else if project.coverImage}
									<img src={optimizedUrlFor(project.coverImage, 200)} alt="" loading="lazy" />
								{/if}
							</span>
						{/if}
					</span>
